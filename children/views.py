...
from .forms import GuardiansForm, ChildrensForm, SignUpForm,AddReportsForm
...
def one_child(request, pk):
    ...
    addReportsForm = AddReportsForm()
    if request.method == "POST":
        addReportsForm = AddReportsForm(request.POST, request.FILES)
        if addReportsForm.is_valid:
            report = addReportsForm.save()
            report.child = child
            report.save()
    return render(request, 'children/one_child.html',
        {"child":child, "addReportsForm":addReportsForm})
